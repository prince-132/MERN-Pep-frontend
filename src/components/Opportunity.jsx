...
const Opportunity = () => {
  ...
  const fetchAppliedOpportunity = async () => {
    try {
      const res = await axios.get('https://mern-pep-backend.onrender.com/auth/all-opportunity');
...
const OpportunityCard = ({ opportunity, appliedOpp }) => {
  ...
  const applyForOpportunity = async (opportunity) => {
    try {
      await axios.post('https://mern-pep-backend.onrender.com/auth/apply', { opportunity })
